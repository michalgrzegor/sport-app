...
import {
  ...
  CalendarArray,
  WeekDate,
} from "../shared/calendar-data.service";
...
import {
  ...
  RepeatData,
} from "./repeat-dialog/repeat-dialog.component";
import {
  trigger,
  state,
  style,
...
import {
  ...
  Association,
  TpInfo,
} from "../shared/store/tiles-data.reducers";
...
export class CalendarSimpleComponent implements OnInit, OnDestroy {
  ...
  expand(calendarIndex: number, weeksIndex: number, weekDatesIndex: number) {
    const day = this.calendarNoDisplayArray.calendar[calendarIndex].weeks[
      weeksIndex
    ].weekDates[weekDatesIndex];
    this._store.dispatch(
      new CalendarDataActions.OpenContainer({
        calendarIndex: calendarIndex,
        weeksIndex: weeksIndex,
        weekDatesIndex: weekDatesIndex,
      })
    );
    this._store.dispatch(
      new TilesDataActions.MakeDay({
        date: day.momentDate,
        day: this.calendarNoDisplayArray.calendar[calendarIndex].weeks[
          weeksIndex
        ].weekDates[weekDatesIndex],
      })
    );
    this._store.dispatch(
      new CalendarDataActions.SetWeek(
        this.calendarNoDisplayArray.calendar[calendarIndex].weeks[
          weeksIndex
        ].weekDates
      )
    );
  ...
  ngOnInit() {
    ...
    this.trainingPlanState = this._store.select(
      (state) => state.tiles.trainingPlan
    );
    this.trainingPlanSub = this.trainingPlanState.subscribe((tp) => {
      let samePlan = false;
      if (this.trainingPlan && tp) {
        if (tp.id === this.trainingPlan.id) {
          samePlan = true;
        }
      }
      if (tp) {
        if (!this.trainingPlan || tp.id !== this.trainingPlan.id) {
          ...
          if (this.openedDay && samePlan) {
            array.calendar[this.openedDay.calendarIndex].weeks[
              this.openedDay.weeksIndex
            ].container[0].expanded = true;
          }
          this._store.dispatch(
            new CalendarDataActions.InitData({
              calendar: array,
              planName: tp.training_plan_name,
            })
          );
        }
        this._store.dispatch(
          new TilesDataActions.SpinnerStartStopCalendar(false)
        );
      }
    ...
    this.athleteAccountonPaidAccountState = this._store.select(
      (state) => state.tiles.athleteAccountonPaidAccount
    );
    ...
    this.noTrialSub = this.noTrialState.subscribe((data) => {
      this.noTrial = data;
      if (!this.trainingPlan && !data) {
    ...
    this.calendarArrayState = this._store.select(
    ...
    this.noTPSub = this.noTPState.subscribe((data) => {
      this.noTP = data;
    ...
    this.athleteAccountState = this._store.select(
      (state) => state.tiles.athleteAccount
    );
    this.athleteAccountSub = this.athleteAccountState.subscribe((data) => {
    ...
    this.onPlatformWithoutPlanState = this._store.select(
      (state) => state.tiles.onPlatformWithoutPlan
    );
    this.onPlatformWithoutPlanSub = this.onPlatformWithoutPlanState.subscribe(
      (data) => (this.onPlatformWithoutPlan = data)
    );

    this.accountLimitState = this._store.select(
      (state) => state.tiles.accountLimit
    );
    this.accountLimitSub = this.accountLimitState.subscribe((data) => {
    ...
    this.loadingState = this._store.select(
      (state) => state.tiles.spinnerCalendar
    );
    this.loadingSub = this.loadingState.subscribe(
      (data) => (this.loading = data)
    );

    this.isPaidAccountState = this._store.select(
      (state) => state.tiles.isPaidAccount
    );
    this.isPaidAccountSub = this.isPaidAccountState.subscribe(
      (data) => (this.isPaidAccount = data)
    );

    this.isOpenBoardState = this._store.select(
      (state) => state.calendar.isOpenTpBoard
    );
    ...
    this.tpManagerSub = this.tpManagerState.subscribe((data: TpInfo[]) => {
    ...
    this.allMomentDateArrayState = this._store.select(
      (state) => state.calendar.allMomentDateArray
    );
    this.allMomentDateArraySub = this.allMomentDateArrayState.subscribe(
      (data) => {
        if (data) {
          this.allMomentDateArray = data;
          this.allMomentDateArray = this.allMomentDateArray.concat(
            this.pushData
          );
        }
      }
    ...
    this.openedDayState = this._store.select(
      (state) => state.calendar.openedDay
    );
    this.openedDaySub = this.openedDayState.subscribe((data) => {
      this.openedDay = data;
      if (data) {
        this.calendarNoDisplayArray.calendar.forEach((month) =>
          month.weeks.forEach((week) => {
            week.container[0].container = false;
            week.container[0].expanded = false;
          })
        );
        this.calendarNoDisplayArray.calendar[data.calendarIndex].weeks[
          data.weeksIndex
        ].container[0].container = true;
        this.calendarNoDisplayArray.calendar[data.calendarIndex].weeks[
          data.weeksIndex
  ...
  doRequests(noTrial: boolean, athleteAccount: boolean) {
    ...
    setTimeout(() => {
      if (
        this.getUserProfile() &&
        JSON.parse(this.getUserProfile())["https://sport.app.comapp_metadata"]
      ) {
        app_metadata = JSON.parse(this.getUserProfile())[
          "https://sport.app.comapp_metadata"
        ];
      }
      ...
      if (
        this.getUserProfile() &&
        app_metadata &&
        app_metadata.account_level_data &&
        app_metadata.account_level_data.account_level === 0 &&
        !athleteAccount &&
        !noTrial
      ) {
        ...
        (this.getUserProfile() &&
          ...
          app_metadata.account_level_data &&
          !app_metadata.account_level_data &&
          JSON.parse(this.getUserProfile())[
            "https://sport.app.comuser_metadata"
          ] &&
          JSON.parse(this.getUserProfile())[
            "https://sport.app.comuser_metadata"
          ].training_plan_last_id &&
          !athleteAccount &&
          !noTrial) ||
        (this.getUserProfile() &&
          app_metadata &&
          app_metadata.account_level_data &&
          app_metadata.account_level_data.account_level === 1 &&
          !app_metadata.account_level_data.current_paid_access_end_date &&
          JSON.parse(this.getUserProfile())[
            "https://sport.app.comuser_metadata"
          ] &&
          JSON.parse(this.getUserProfile())[
  ...
  drop(
    ...
    this.calendarNoDisplayArray.calendar[calendarIndex].weeks[
      weeksIndex
    ].weekDates[weekDatesIndex].association.forEach((asso) => {
    ...
    if (arrayOne.length > 0) {
      index = arrayOne[arrayOne.length - 1].asso_index_in_array + 1;
    } else {
      index = 1;
    }
    ...
    if (this.openedDay) {
      isOpenedDay = true;
      openedDayDate = this.calendarNoDisplayArray.calendar[
        this.openedDay.calendarIndex
      ].weeks[this.openedDay.weeksIndex].weekDates[
        this.openedDay.weekDatesIndex
      ].momentDate;
      day.date = this.calendarNoDisplayArray.calendar[
        this.openedDay.calendarIndex
      ].weeks[this.openedDay.weeksIndex].weekDates[
        this.openedDay.weekDatesIndex
      ].momentDate;
      this.calendarNoDisplayArray.calendar[this.openedDay.calendarIndex].weeks[
        this.openedDay.weeksIndex
      ].weekDates[this.openedDay.weekDatesIndex].association.forEach((asso) =>
        day.day.association.push(asso)
      );
      if (
        this.calendarNoDisplayArray.calendar[this.openedDay.calendarIndex]
          .weeks[this.openedDay.weeksIndex].weekDates[
          this.openedDay.weekDatesIndex
        ].momentDate === daya.momentDate
      ) {
        day.day.association.push(asso);
      }
    }

    this._httpCalendarService.postCalendarAssoc(
      this.trainingPlan,
      ...
      isOpenedDay,
      openedDayDate,
      this.calendarNoDisplayArray.calendar[calendarIndex].weeks[weeksIndex]
        .weekDates
    );
    ...
    let snackBarRef = this._snackBar.open(
      `Do you want to repeat this action?`,
      `REPEAT`,
      { duration: 6000 }
    );
    ...
    snackBarRef
      .onAction()
      .subscribe(() =>
        this.openDialog(asso, calendarIndex, weeksIndex, weekDatesIndex)
      );
  }

  makeStars(calendar: CalendarArray, tp: TrainingPlan) {
    calendar.calendar.forEach((calendar) =>
      calendar.weeks.forEach((week) =>
        week.weekDates.forEach((date) => {
          tp.calendar_stars.forEach((star) => {
            if (
              moment(star.star_date).format() ===
              moment(date.momentDate).format()
            ) {
  ...
  add() {
    ...
    if (this.i < limit - 1) {
      ...
      if (this.openedDay) {
        this._store.dispatch(
          new CalendarDataActions.CloseContainer({
            calendarIndex: this.openedDay.calendarIndex,
            weeksIndex: this.openedDay.weeksIndex,
            weekDatesIndex: this.openedDay.weekDatesIndex,
          })
        );
      }
    }
  ...
  repeatAction(
    ...
    if (data.weekly) {
      ...
      let nRW = 0;
      const limitRW =
        calendarRW.calendar.length * 6 - (calendarNumberRW * 6 + weeksNumberRW);
      const newAssoArrayRW = [];
      for (let i = 1; i <= Number(data.weekly) + nRW && i < limitRW; i++) {
        if (
          calendarRW.calendar[
            calendarNumberRW + Math.floor((weeksNumberRW + i) / 6)
          ].weeks[weeksNumberRW + i - 6 * Math.floor((weeksNumberRW + i) / 6)]
            .weekDates[weekDatesNumberRW].available
        ) {
          ...
          asso.calendar_date =
            calendarRW.calendar[
              ...
              weeksNumberRW + i - 6 * Math.floor((weeksNumberRW + i) / 6)
            ].weekDates[weekDatesNumberRW].momentDate;
          calendarRW.calendar[
            calendarNumberRW + Math.floor((weeksNumberRW + i) / 6)
          ].weeks[
            weeksNumberRW + i - 6 * Math.floor((weeksNumberRW + i) / 6)
          ].weekDates[weekDatesNumberRW].association.push(asso);
      ...
      this._httpCalendarService.repeatWeaklyAsso(
        this.trainingPlan.id,
        newAssoArrayRW,
        calendarRW,
        tp,
        ...
        openedDayDate,
        this.calendarArray,
        this.calendarNoDisplayArray.calendar[calendarIndexV].weeks[weeksIndex]
          .weekDates
      );
    } else if (data.daily) {
      const tp: TrainingPlan = Object.assign({}, this.trainingPlan);
      ...
      let nRD = 0;
      const limitRD =
        calendarRD.calendar.length * 6 * 7 -
        calendarNumberRD * 6 * 7 -
        weeksNumberRD * 7 -
        weekDatesNumberRD -
        7;
      ...
      for (let i = 1; i <= Number(data.daily) + nRD && i < limitRD; i++) {
        let a =
          calendarNumberRD +
          Math.floor(
            (weeksNumberRD + Math.floor((weekDatesNumberRD + i) / 7)) / 6
          );
        let b =
          weeksNumberRD +
          Math.floor((weekDatesNumberRD + i) / 7) -
          6 *
            Math.floor(
              (weeksNumberRD + Math.floor((weekDatesNumberRD + i) / 7)) / 6
            );
        let c =
          weekDatesNumberRD + i - 7 * Math.floor((weekDatesNumberRD + i) / 7);
        if (calendarRD.calendar[a].weeks[b].weekDates[c].available) {
          let asso = Object.assign({}, assos);
          asso.calendar_date =
            calendarRD.calendar[a].weeks[b].weekDates[c].momentDate;
      ...
      this._httpCalendarService.repeatDailyAsso(
        this.trainingPlan.id,
        newAssoArrayRD,
        calendarRD,
        tp,
  ...
  makeWeek(calendarIndex: number, weeksIndex: number) {
    ...
    this.calendarNoDisplayArray.calendar[calendarIndex].weeks[
      ...
      if (week.association) {
        week.association.forEach((asso) => assoArray.push(asso));
      }
    });
  ...
  openTPEditor() {
    ...
    if (this.accountLimit * 10 > tpManagerLength) {
      ...
      const dialogRef = this._dialog.open(TrainingPlanEditorComponent, {
        ...
        width: `408px`,
        data: {
          training_plan_name: this.training_plan_name,
          date_from: this.date_from,
          date_to: this.date_to,
          training_sesion_number: this.training_sesion_number,
          isEdited: false,
        },
      });
      ...
      this.openSnackBar(
        `You reached limit. You can upgrade Your Subscription Plan or delete inactive training plan`
      );
    }
  }
